...
namespace IntroSatLib {
    ...
    ISL_StatusTypeDef UARTDevice::WriteUART(uint8_t* buf, uint16_t nBytes, uint16_t timeout) {
        return _uart.transmit(buf, nBytes, timeout);
    }

    ISL_StatusTypeDef UARTDevice::ReadUART(uint8_t* buf, uint16_t nBytes, uint16_t timeout) {
